...
##############################
# 4a) OPDATER ET EKSISTERENDE ITEM
...
def update_item(item_pk, lan="dk"):
    ...
    # 2) Hent og valider de indsendte formular-felter
    item_name    = request.form["item_name"].strip()
    item_address = request.form["item_address"].strip()
    item_price   = request.form["item_price"].strip()
...
def update_user(lan="dk"):
    ...
    # Opdater session-objektet, så visning af navn/email er korrekt
    session["user"].update({
      "user_name":      user_name,
...
##############################
# 3) SOFT-DELETE AF BRUGER (“slet profil”)
@app.delete("/user")
@app.delete("/<lan>/user")
